...
@register_vfe
class PillarFeatureNetMMS(nn.Module):
    def __init__(self,
                 num_input_features=4,
                 use_norm=True,
                 num_filters=(64,),
    ...
    def forward(self, pillar_x, pillar_y, pillar_z, pillar_i, num_voxels,
        ...
        # Find distance of x, y, and z from pillar center
        f_center_offset_x = pillar_x - x_sub_shaped
        f_center_offset_y = pillar_y - y_sub_shaped

        f_center_concat = torch.cat((f_center_offset_x, f_center_offset_y), 1)

        pillar_xyzi = torch.cat((pillar_x, pillar_y, pillar_z, pillar_i), 1)
        features_ls = [pillar_xyzi, f_cluster, f_center_concat]
...
@register_middle
class PointPillarsScatterMMS(nn.Module):
    def __init__(self,
                 output_shape,
                 num_input_features=64,
                 batch_size = 1):
        ...
        super().__init__()
        self.name = 'PointPillarsScatterMMS'
        self.output_shape = output_shape
        self.ny = output_shape[2]
        self.nx = output_shape[3]
        self.nchannels = num_input_features
